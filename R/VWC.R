...
d <- downloadTOA5("WTC_AUTO_C[0-9]{1,2}_SOILVARS", startDate="2015-5-1", endDate="2016-11-30",
...
d2 <- downloadTOA5("WTC_AUTO_C[0-9]{1,2}_XTRATDR", startDate="2015-5-1", endDate="2016-11-30",
                  topath="C:/Repos/wtc4_flux/data/fromHIEv",maxnfiles=100,
...
dfr.day.treat <- summaryBy(.~Date+combotrt,data=subset(dfr.day2,Date>as.Date("2016-9-01")),
                           FUN=c(mean,standard.error),keep.names=T,na.rm=T)
...
plotBy(VW_Avg.1..mean~Date|combotrt,data=dfr.day.treat,type="l",legend=F,ylim=c(0,0.3),col=c("blue","red"),
...
plotBy(VW_surface.mean~Date|combotrt,data=dfr.day.treat,type="l",legend=F,ylim=c(0,0.3),
       ylab="VWC- three surface",lwd=3)
legend("topleft",levels(dfr.day.treat$combotrt),lty=1,lwd=2,ncol=2,col=palette()[1:4])
#adderrorbars(x=dfr.day.a$Date,y=dfr.day.a$VW_surface.mean,SE=dfr.day.a$VW_surface.standard.error,
#             direction="updown",col="blue",barlen=0)
#adderrorbars(x=dfr.day.e$Date,y=dfr.day.e$VW_surface.mean,SE=dfr.day.e$VW_surface.standard.error,
#             direction="updown",col="red",barlen=0)



#- mid depth
plotBy(VW_Avg.2..mean~Date|combotrt,data=dfr.day.treat,type="l",legend=F,ylim=c(0,0.3),
       ylab="VWC- mid",lwd=3)
#adderrorbars(x=dfr.day.a$Date,y=dfr.day.a$VW_Avg.2..mean,SE=dfr.day.a$VW_Avg.2..standard.error,
#             direction="updown",col="blue",barlen=0)
#adderrorbars(x=dfr.day.e$Date,y=dfr.day.e$VW_Avg.2..mean,SE=dfr.day.e$VW_Avg.2..standard.error,
#             direction="updown",col="red",barlen=0)

#- deep
plotBy(VW_Avg.3..mean~Date|combotrt,data=dfr.day.treat,type="l",legend=F,ylim=c(0,0.3),
       ylab="VWC- deep",lwd=3)
adderrorbars(x=dfr.day.a$Date,y=dfr.day.a$VW_Avg.3..mean,SE=dfr.day.a$VW_Avg.3..standard.error,
             direction="updown",col="blue",barlen=0)
adderrorbars(x=dfr.day.e$Date,y=dfr.day.e$VW_Avg.3..mean,SE=dfr.day.e$VW_Avg.3..standard.error,
             direction="updown",col="red",barlen=0)
...
#- daily chamber means
plotBy(SoilTempProbe_Avg.2.~Date|chamber,data=dfr.day,type="o",legend=F,ylim=c(0,35))
legend("top",pch=16,lty=1,legend=levels(dfr.day$chamber),col=palette()[1:12],ncol=6)

#- daily treatment means
plotBy(SoilTempProbe_Avg.2.~Date|T_treatment,col=c("blue","red"),data=dfr.day.treat,type="o",legend=F,ylim=c(0,35))
legend("top",pch=16,lty=1,legend=levels(dfr.day.treat$T_treatment),col=c("blue","red"))
