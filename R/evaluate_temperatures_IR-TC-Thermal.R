...
windows(100,35)
par(mfrow=c(1,3),las=1,cex.lab=2,mar=c(6,6,1,1))
...
greys.ramp <- colorRampPalette(c("whitesmoke", "black"))



#- create color densities
dc1 <- densCols(subset(Tdat_hr,PAR>parlimit)$TargTempC_Avg,subset(Tdat_hr,PAR>parlimit)$Tair_al,
               colramp=greys.ramp)
dc2 <- densCols(subset(Tdat_hr,PAR>parlimit)$Tleaf,subset(Tdat_hr,PAR>parlimit)$Tair_al,
                colramp=greys.ramp)
dc3 <- densCols(subset(Tdat_hr,PAR>parlimit)$TargTempC_Avg,subset(Tdat_hr,PAR>parlimit)$Tair_al,
                colramp=greys.ramp)

#- plot leaf temperatures relative to air temperature
plot(TargTempC_Avg~Tair_al,data=subset(Tdat_hr,PAR>parlimit),ylim=lims,xlim=lims,pch=16,
     col=dc1,
     xlab=expression(Air~temperature~(T[air]*","~degree*C)),
     ylab=expression(Infrared~leaf~temperature~(T[l-IR]*","~degree*C)));abline(0,1,col="grey",lty=2)
lm1 <- lm(TargTempC_Avg~Tair_al,data=subset(Tdat_hr,PAR>parlimit))
lines(predict.lm(lm1,newdat=data.frame(Tair_al=seq(0,35,length.out=101))) ~ seq(0,35,length.out=101),
      lwd=1.5,col="black")
legend("topleft",legend=letters[1],cex=1.4,bty="n")

plot(Tleaf~Tair_al,data=subset(Tdat_hr,PAR>parlimit),ylim=lims,xlim=lims,pch=16,
     col=dc2,
     xlab=expression(Air~temperature~(T[air]*","~degree*C)),
     ylab=expression(Thermocouple~leaf~temperature~(T[l-TC]*","~degree*C)));abline(0,1,col="grey",lty=2)
#points(LeafT_Avg.2.~Tair_al,data=subset(Tdat_hr,PAR>parlimit),pch=3)
lm2 <- lm(Tleaf~Tair_al,data=subset(Tdat_hr,PAR>parlimit))
lines(predict.lm(lm2,newdat=data.frame(Tair_al=seq(0,35,length.out=101))) ~ seq(0,35,length.out=101),
        lwd=1.5,col="black")
legend("topleft",legend=letters[2],cex=1.4,bty="n")


#- plot covariance between temperature measurements
plot(Tleaf~TargTempC_Avg,data=subset(Tdat_hr,PAR>parlimit),ylim=lims,xlim=lims,pch=16,
     col=dc3,
     xlab=expression(Infrared~leaf~temperature~(T[l-IR]*","~degree*C)),
     ylab=expression(Thermocouple~leaf~temperature~(T[l-TC]*","~degree*C)));abline(0,1,col="grey",lty=2)
lm3 <- lm(Tleaf~TargTempC_Avg,data=subset(Tdat_hr,PAR>parlimit))
lines(predict.lm(lm3,newdat=data.frame(TargTempC_Avg=seq(0,35,length.out=101))) ~ seq(0,35,length.out=101),
      lwd=1.5,col="black")
confint(lm3)
...
windows(80,35)
par(mfrow=c(1,2),las=1,cex.lab=1.5,cex.axis=1.1,mar=c(6,7,1,5))
...
axis(side=4)
title(ylab=expression(PPFD~on~leaf~surface~(mu*mol~m^-2~s^-1)),xpd=NA,line=-35)
...
plot(Tdiff_IR~PAR,data=subset(Tdat_hr,PAR>4),ylim=c(-5,15),xlim=c(0,2000),pch=3,col="black",
     xlab=expression(Top-Canopy~PPFD~(mu*mol~m^-2~s^-1)),
     ylab=expression(T[leaf]-T[air]~(degree*C)));abline(h=0)
points(Tdiff_TC~PAR,data=Tdat_hr,pch=3,col="blue")
legend("bottomright",pch=3,col=c("black","blue","red"),legend=c("Infrared","Thermocouple","Thermal images"))
...
points(Tdiff_Thermo~PAR,data=TCdat.all,pch=3,col="red")
